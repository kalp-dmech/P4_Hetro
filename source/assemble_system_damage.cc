      ...
      if (cell->is_locally_owned ())
       {
        ...
        for (const unsigned int q_index : fe_values_damage.quadrature_point_indices())
        {
            ...
            double H_call = H_plus(strain_values[q_index], material);
            ...
            for (const unsigned int i : fe_values_damage.dof_indices())
            {
                ...
                cell_rhs_damage(i) += (2*lo/material.Gc*H*N_v_i)*fe_values_damage.JxW(q_index);
               ...
               for (const unsigned int j : fe_values_damage.dof_indices())
                {
                   ...
                   cell_matrix_damage(i, j) += ((lo*lo*grad_N_v_i*grad_N_phi_j) + (1 + (2*H*lo)/material.Gc) * (N_v_i*N_phi_j)) * fe_values_damage.JxW(q_index);
                }
            }
        }
