        ...
        for (unsigned int face_number = 0; face_number < GeometryInfo<2>::faces_per_cell; ++face_number)
        {
                ...
                cell->face(face_number)->boundary_id() == top_boundary_id)
            {
                ...
                for (unsigned int q = 0; q < n_q_f_points; ++q)
                {
                    ...
                    // Apply degradation function (1-phi)^2
                    double degradation = (1.0 - phi) * (1.0 - phi);

                    // Calculate stresses with degradation
                    double sxy = degradation * 2.0 * mu * exy;
                    double syy = degradation * (lambda * (exx + eyy) + 2.0 * mu * eyy);
